...
window.FerryPractice = (() => {
  ...
  function renderStory() {
    ...
    const label = a.replay ? "回想 · 不影響今日進度" : story.theme;
    const next = title => button("next",title,"",true);
    let body = "", controls = [], title = story.title;
    if (a.phase === "intro") {body = paragraph(story.opening); controls = [next("走近看看")];}
    if (a.phase === "observe") {
      ...
      controls = [next(a.seen.length ? "帶著看見的，往前一步" : "現在就想想能做什麼")];
    }
    ...
    if (a.phase === "effect" && action) {title = "眼前，已經有了不同"; body = paragraph(action.effect); controls = [next("看看還不能決定的部分")];}
    if (a.phase === "release") {title = "把餘下的，留給河流"; body = paragraph(story.release); if (a.replay && action) body += `<section class="practice-letter"><span>回想裡的後來 · 正式故事隔日來信</span>${paragraph(action.later)}</section>`; controls = [next("把一小步帶回去")];}
    if (a.phase === "carry") {
      title = "生活裡，也可以試一小步";
      body = `<p>不必交功課，也不用證明有用。<br>如果願意，留下一個你認得的時刻。</p><label class="practice-field">當什麼事情發生時<input id="lifeWhen" maxlength="90" value="${esc(a.draft?.when ?? story.context)}"></label><label class="practice-field">我願意先做的一小步<textarea id="lifeStep" maxlength="160" rows="3">${esc(a.draft?.step ?? story.step)}</textarea></label><p class="practice-note">只存在這個瀏覽器。請不要填入敏感個資。${a.replay ? "回想不會另外建立生活紀錄。" : ""}</p>`;
      controls = a.replay ? [button("finish:skip","讓這段回想歇下","",true)] : [button("finish:keep","帶著這一步離開","",true),button("finish:skip","今天先不帶，也可以")];
    }
    paint(label,title,body,controls);
    if(a.phase === "carry") for(const id of ["lifeWhen","lifeStep"]) $(id).addEventListener("input",()=>{
  ...
  function replays() {view="replays"; paint("自由回想","換一個角度，再看看",paragraph("這裡可以隨時遊玩完整故事與後續，不計入今日局，也不會給答對獎勵。"),PRACTICE_STORIES.map(x=>button("replay:"+x.id,x.title,x.theme)));}
  function mail() {
    view="mail";
  ...
  async function dispatch(key) {
    ...
    if(key==="next") {if(commit(C.advance(S)))renderStory();return;}
    ...
    if(key.startsWith("finish:")) {
      ...
      if(commit(C.finish(S,todayStr(),storyOf(),invitation))) {view="done";paint("今天，到這裡就好","船已靠岸",paragraph("不用等心情變好才離開。\n下一次在生活裡想起一小步，就從那裡開始。"),[button("home","回到渡口","",true),button("replays","再回想另一段故事")]);}return;
    }
