...
test('five trials have 40 playable paths; feelings never determine external outcomes',()=>{
  ...
  for(const c of C.cases){assert.equal(c.reactions.length,3);assert.equal(c.actions.length,2);const results=[new Set(),new Set()];
    for(const reaction of [null,...c.reactions.map(r=>r.id)])for(const [i,a]of c.actions.entries()){
      let s=C.create(c.id);const original=JSON.stringify(s);
      if(reaction){s=C.transition(s,{type:'choose',id:reaction});assert.ok(C.valid(s));assert.equal(s.phase,'flip');assert.match(C.sceneFor(s).text,/不貼近你也沒關係/);s=C.transition(s,{type:'continue'});}else s=C.transition(s,{type:'skip'});
...
test('trial validation and transitions reject unknown and inconsistent choices',()=>{
  const s=C.create();for(const bad of [null,{},[],{...s,extra:1},{...s,caseId:'bad'},{...s,seen:[]},{...s,seen:[s.caseId,s.caseId]},{...s,phase:'flip'},{...s,action:'a0'},{...s,reaction:'t0'},{...s,phase:'ending'},{...s,phase:'response',reaction:'bad'}])assert.equal(C.valid(bad),false);
  for(const event of [null,{type:'continue'},{type:'next'},{type:'choose',id:'a0'}])assert.equal(C.transition(s,event),s);
  const flip=C.transition(s,{type:'choose',id:'t0'});assert.deepEqual(C.transition(flip,{type:'back'}),s);assert.equal(C.transition(flip,{type:'skip'}),flip);
...
test('all 40 controller paths, skip, back, reflections, replay and rest work without touching storage',async()=>{
  for(const c of C.cases)for(const reaction of [-1,0,1,2])for(const action of [0,1]){
    ...
    if(reaction<0)await e.confirm.emit('click');else{
      await e.hand.children[reaction].emit('click');assert.equal(e.game.dataset.phase,'flip');assert.equal(e.choicesArea.hidden,true);
      await e.backStory.emit('click');assert.equal(e.game.dataset.phase,'scene');await e.hand.children[reaction].emit('click');await e.confirm.emit('click');
    }
...
test('trial reload resets only trial; short readers paginate losslessly and viewport retains selection',async()=>{
  ...
  assert.equal(await s.readAll(),C.cases[0].event.text);await e.hand.children[1].emit('click');assert.match(await s.readAll(),/我整個人很差/);
