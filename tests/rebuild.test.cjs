...
test('activation removes only old scoped caches and redirects only ferry tabs without deadlock',async()=>{
  const keys=[prefix+'boat-v1',prefix+'rebuild-v1',prefix+'scenarios-v8','du-ferry:https://example.com/other/:boat-v1','unrelated-cache'];
...
test('fresh install and future card updates do not force-navigation of current story tabs',async()=>{
  for(const keys of [[prefix+'scenarios-v8'],[prefix+'cards-v5',prefix+'cards-v6',prefix+'light-v1',prefix+'light-v2',prefix+'scenarios-v0',prefix+'scenarios-v1',prefix+'scenarios-v8']]){
    const w=worker({keys});let done;w.handlers.activate({waitUntil:p=>done=p});await done;assert.deepEqual(w.navigated,[]);assert.deepEqual(w.deleted,keys.filter(k=>k!==prefix+'scenarios-v8'));
  }
});
