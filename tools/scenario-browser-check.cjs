...
const C=require('../scenario-core.js');
...
(async()=>{
  ...
  try{
    ...
    await page.goto(url);await page.waitForFunction(()=>window.FerryScenarioCore);
    ...
    for(const [width,height,font]of sizes){
      ...
      for(const id of C.categories.map(c=>C.create(c.id,()=>0).caseId)){
        for(const phase of ['scene','response','ending']){
          let state=({...C.create(),caseId:id,seen:[id]});if(phase!=='scene')state=C.transition(state,{type:'choose',id:'a1'});if(phase==='ending')state=C.transition(state,{type:'choose',id:'r1'});
          await page.evaluate(s=>localStorage.setItem('du_ferry_scenarios_v1',JSON.stringify(s)),state);await page.reload();
    ...
    await page.setViewportSize({width:390,height:844});await page.evaluate(()=>localStorage.removeItem('du_ferry_scenarios_v1'));await page.reload();await inspect('random first draw');
    const screenshot=path.join(os.tmpdir(),'ferry-scenario-preview-390.png');await page.screenshot({path:screenshot});
    await page.locator('#hand button').first().click();await page.locator('#confirm').click();await page.locator('#hand button').last().click();await page.locator('#confirm').click();await page.locator('#rest').click();await inspect('rest');
