...
(async()=>{
  ...
  try{
    ...
    assert.equal(await page.evaluate(()=>JSON.parse(localStorage.getItem('du_ferry_light_v1')).phase),'ending');
    ...
    await page.goto((process.argv[2]||'http://127.0.0.1:8138/')+'boat.html');assert.equal(await page.locator('#sceneTitle').textContent(),'一小塊暖的地方');
