...
const assert=require('node:assert/strict'),C=require('../light-core.js');
(async()=>{
  ...
  try{
    for(const visual of [null,620,500]){
      ...
      await inspect('prologue/'+visual);
      for(const id of ['arrival','return','last-mirror']){
        await page.evaluate(s=>localStorage.setItem('du_ferry_light_v1',JSON.stringify(s)),C.create(id,false));await page.reload();await inspect(id+'/'+visual);
        ...
        if(visual){
          const raw=await page.evaluate(()=>localStorage.getItem('du_ferry_light_v1'));
          ...
          assert.equal(await page.evaluate(()=>localStorage.getItem('du_ferry_light_v1')),raw);
