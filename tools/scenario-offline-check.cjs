...
(async()=>{
  ...
  try{
    ...
    const cached=await page.evaluate(async()=>{const names=await caches.keys();return {names,files:(await (await caches.open(names.find(n=>n.endsWith('scenarios-v2')))).keys()).map(r=>new URL(r.url).pathname)};});
