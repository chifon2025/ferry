...
(async()=>{
  ...
  try{
    ...
    const cached=await page.evaluate(async()=>{const names=await caches.keys();return {names,files:(await (await caches.open(names.find(n=>n.endsWith('scenarios-v1')))).keys()).map(r=>new URL(r.url).pathname)};});
    assert.equal(cached.files.length,13);assert.ok(cached.files.some(f=>f.endsWith('scenario-seeds.js')));assert.ok(!cached.files.some(f=>f.includes('flower')));
