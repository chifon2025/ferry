...
(function(root){
  ...
  function sceneFor(s){
    ...
    if(s.phase==='flip'){const chosen=c.reactions.filter(r=>s.reactions.includes(r.id));return {title:chosen.length===1?'「'+chosen[0].title+'」':'幾種心情，可以同時存在',text:chosen.map(r=>'「'+r.title+'」\n'+r.text).join('\n\n')+'\n\n我可以看見這些念頭，不用急著跟著它們走。\n\n這只是另一個角度，不貼近你也沒關係。還沒平靜，也可以選下一步。'};}
