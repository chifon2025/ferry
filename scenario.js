(() => {
  'use strict';
  const C=window.FerryScenarioCore,L=window.FerryCardLayout,$=id=>document.getElementById(id),KEY='du_ferry_scenarios_v1';
  ...
  function phaseLabel(){return {scene:'遇見 · 先看眼前',response:'後續 · 再選一步',ending:'此刻 · 先到這裡'}[state.phase];}
  function render(focus=false){
    ...
    $('confirm').textContent=state.phase==='ending'?'再遇見一件事':'先選一張牌';
    $('footerNote').textContent=writable?'不計分。同一輪優先遇見尚未玩過的事。':'本次暫不保存；關閉後可能需要重新開始。';
  ...
  $('confirm').addEventListener('click',()=>{
    if(view){closeView();return;}
    if(state.phase==='ending')dispatch({type:'next'});
